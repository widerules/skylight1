...
public class OpenGLGeometryBuilder {
	...
	public class Triangle3D {
		public Triangle3D setTextureCoordinates(float aU1, float aV1, float aU2, float aV2, float aU3, float aV3) {
		...
		public Triangle3D setNormal(float aNormalX1, float aNormalY1, float aNormalZ1, float aNormalX2,
				float aNormalY2, float aNormalZ2, float aNormalX3, float aNormalY3, float aNormalZ3) {
			if (!usesNormals) {
		...
		public Triangle3D setColour(float aRed1, float aGreen1, float aBlue1, float aRed2, float aGreen2, float aBlue2,
	...
	public class Rectangle2D {
		public Rectangle2D setTextureCoordinates(float aU, float aV, float aUWidth, float aVHeight) {
			if (!usesTextureCoordinates) {
				throw new IllegalStateException("This OpenGLGeometryBuilder is not configured for textures");
			}
		...
		public Rectangle2D setColour(float aRed1, float aGreen1, float aBlue1, float aRed2, float aGreen2,
				float aBlue2, float aRed3, float aGreen3, float aBlue3, float aRed4, float aGreen4, float aBlue4) {
			if (!usesColours) {
				throw new IllegalStateException("This OpenGLGeometryBuilder is not configured for colours");
			}
	...
	private final Triangle3D triangle3D = new Triangle3D();

	private final Rectangle2D rectangle2D = new Rectangle2D();
	 ...
	 */
	public Triangle3D add3DTriangle(float anX1, float aY1, float aZ1, float anX2, float aY2, float aZ2, float anX3,
			float aY3, float aZ3) {
	 ...
	 *         OpenGLGeometryBuilder
	 */
	public Rectangle2D add2DRectangle(float anX, float aY, float aWidth, float aHeight, float aTextureX,
			float aTextureY, float aTextureWidth, float aTextureHeight) {
		checkCurrentGeometryAndCompleteAndCheckAndSetMode(GL10.GL_TRIANGLES);
