...
public class LogHandlerServlet extends HttpServlet
{
	...
	throws ServletException, IOException
	{
		...
		ExceptionUnmarshaller unm = new ExceptionUnmarshaller();
		ExceptionClass exc = null;
		try {
			exc = unm.unmarshall(st);
		} catch (JAXBException e) {
			getServletContext().log("Data could not be parsed to Exception XML Syntax : '" + st + "'");
			e.printStackTrace();
			return;
		}

		PersistingException persist = new PersistingException(exc);
		pm.makePersistent(persist);

        response.setContentType("text/html");
        String source = request.getHeader("User-Agent");
        if(request.getParameter("emailtest")!=null) { // to test email with GET
        	source = request.getParameter("emailtest");
        }

        final String host = getServletConfig().getInitParameter("host");
        final String to = getServletConfig().getInitParameter(source); // select destination based on agent
        ...
        if(host!=null) {
	        ...
	        try
	        {
	            ...
	            //  getServletContext().log("mail sent: >>"+message.toString()+"<<");
	...
	private void sendMail(String host, String to, String from, String subject, String content) throws Exception {
		...
		Authenticator auth = new SMTPAuthenticator(
				...
				getServletConfig().getInitParameter("pwd")
				);
